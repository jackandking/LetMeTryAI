...
const questionConfig = {
    title: '家里电价上升，你会怎样调整用电？（家长实用篇）',
    question: '家里电价上升，你会怎样调整用电？',
    options: [
        { value: 'batch-cooking-laundry', label: '集中做饭与洗衣（一次性完成）' },
        { value: 'shift-to-night', label: '错峰使用（更多夜间或低价时段）' },
        { value: 'use-energy-saving-devices', label: '更新节能设备&用智能插座' },
        { value: 'reduce-waste-through-habits', label: '以习惯教育为主，减少浪费' }
    ],
    storageKey: 'home_electricity_adjustment.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=home-electricity-adjustment'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家里电价上升，你会怎样调整用电？（家长实用篇）结果</h2>';
