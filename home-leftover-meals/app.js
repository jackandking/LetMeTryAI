...
const questionConfig = {
    title: '家里剩饭你怎么处理？留着热一热、改造成新菜，还是直接处理掉？',
    question: '家里剩饭你怎么处理？',
    options: [
        { value: 'reheat-eat', label: '留着加热再吃' },
        { value: 'transform-new-dish', label: '改造成新菜' },
        { value: 'freeze-portion', label: '分装冷冻备用' },
        { value: 'compost-or-trash', label: '不吃直接处理(堆肥/丢弃)' }
    ],
    storageKey: 'home_leftover_meals.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=home-leftover-meals'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家里剩饭你怎么处理？留着热一热、改造成新菜，还是直接处理掉？结果</h2>';
