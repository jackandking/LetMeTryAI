...
const questionConfig = {
    title: '神舟发射时哪个画面把你看呆了？',
    question: '神舟发射时哪个画面把你看呆了？',
    options: [
        { value: 'ignition-flame', label: '点火瞬间·火焰冲天' },
        { value: 'stage-separation', label: '分离瞬间·火花与烟雾' },
        { value: 'smoke-trail', label: '尾迹划天·白色长龙' },
        { value: 'astronaut-closeup', label: '航天员/指挥近景·表情瞬间' }
    ],
    storageKey: 'shenzhou_launch_scenes.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=shenzhou-launch-scenes'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>神舟发射时哪个画面把你看呆了？结果</h2>';
