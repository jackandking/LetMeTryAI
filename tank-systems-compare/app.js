...
const questionConfig = {
    title: '主战坦克：火力、装甲、机动哪个影响实际表现？',
    question: '选择坦克时，你最看重哪个指标？',
    options: [
        { value: 'heavy-firepower', label: '火力优先（强打击）' },
        { value: 'balanced-protection', label: '装甲优先（防护强）' },
        { value: 'high-mobility', label: '机动优先（快速机动）' }
    ],
    storageKey: 'tank_systems_compare.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=tank-systems-compare'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>主战坦克：火力、装甲、机动哪个影响实际表现？结果</h2>';
