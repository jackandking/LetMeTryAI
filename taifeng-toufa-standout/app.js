...
const questionConfig = {
    title: '台风天头发还能看？哪款发型更能站住场面：惊艳背影还是稳妥收拢？',
    question: '台风天头发还能看？在暴风里你更能接受哪款发型站住场面？',
    options: [
        { value: 'high-top-bun', label: '高位丸子头' },
        { value: 'low-sleek-ponytail', label: '紧贴低马尾' },
        { value: 'shoulder-waves', label: '披肩大波浪（定型）' },
        { value: 'air-bangs-short', label: '短发＋空气刘海' }
    ],
    storageKey: 'taifeng_toufa_standout.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=taifeng-toufa-standout'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>台风天头发还能看？哪款发型更能站住场面：惊艳背影还是稳妥收拢？结果</h2>';
