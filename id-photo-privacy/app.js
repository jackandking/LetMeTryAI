...
const questionConfig = {
    title: '身份证照片如何保护隐私',
    question: '你认为在分享或保存身份证照片时，哪种做法最能兼顾隐私与实用性？',
    options: [
        { value: 'mask-sensitive-info', label: '遮挡敏感信息（黑条/涂抹）' },
        { value: 'blur-photo', label: '模糊或马赛克处理整张照片' },
        { value: 'partial-or-distance', label: '部分拍摄或远距拍摄' },
        { value: 'watermark-or-encrypt', label: '添加水印/使用加密与受限查看' }
    ],
    storageKey: 'id_photo_privacy.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=id-photo-privacy'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>身份证照片如何保护隐私结果</h2>';
