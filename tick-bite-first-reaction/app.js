...
const questionConfig = {
    title: '夏天被蜱虫叮了，你头一个反应是啥？',
    question: '夏天被蜱虫叮了，你头一个反应是啥？',
    options: [
        { value: 'panic-pull-by-hand', label: '慌了神，想用手硬拔' },
        { value: 'disinfect-first', label: '先消毒，再慢慢处理' },
        { value: 'tweezers-vertical', label: '用镊子垂直夹出来' },
        { value: 'go-to-hospital', label: '不敢乱动，直接去医院' }
    ],
    storageKey: 'tick_bite_first_reaction.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=tick-bite-first-reaction'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>夏天被蜱虫叮了，你头一个反应是啥？结果</h2>';
