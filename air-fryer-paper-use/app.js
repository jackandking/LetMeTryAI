...
const questionConfig = {
    title: '空气炸锅用纸，你会用吗？',
    question: '空气炸锅用纸，你会用吗？',
    options: [
        { value: 'always-use-paper', label: '每次都用' },
        { value: 'use-when-greasy', label: '遇到油多或易粘就用' },
        { value: 'use-foil-or-oil', label: '不用纸，改用铝箔或刷薄油' },
        { value: 'never-use-paper', label: '从不用（觉得浪费或担心安全）' }
    ],
    storageKey: 'air_fryer_paper_use.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=air-fryer-paper-use'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>空气炸锅用纸，你会用吗？结果</h2>';
