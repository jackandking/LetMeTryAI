...
const questionConfig = {
    title: '如何安全分享身份证照片：长辈可选的实用做法',
    question: '遇到需要提供身份证信息时，你更倾向哪种分享方式？',
    options: [
        { value: 'remove-sensitive-info', label: '去敏后截图' },
        { value: 'watermark-and-limit', label: '加水印并限制对象' },
        { value: 'in-person-verification', label: '当面核实不发图' },
        { value: 'official-secure-upload', label: '通过官方或加密渠道上传' }
    ],
    storageKey: 'how_to_share_id_safely.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=how-to-share-id-safely'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>如何安全分享身份证照片：长辈可选的实用做法结果</h2>';
