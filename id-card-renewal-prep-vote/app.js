...
const questionConfig = {
    title: '居民身份证换证准备方式投票',
    question: '如果要去办理居民身份证换证，你通常会怎样准备？',
    options: [
        { value: 'advance-appointment-prepare', label: '提前预约并按清单准备材料' },
        { value: 'walk-in-same-day', label: '当天到现场临时办理并排队' },
        { value: 'entrust-family', label: '委托子女或亲友代办' },
        { value: 'community-home-service', label: '使用社区或代办平台的上门/集中代办服务' }
    ],
    storageKey: 'id_card_renewal_prep_vote.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=id-card-renewal-prep-vote'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>居民身份证换证准备方式投票结果</h2>';
