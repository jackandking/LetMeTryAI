...
const questionConfig = {
    title: '面对车企降价你会怎么选？',
    question: '面对车企降价，你会如何做出选择？',
    options: [
        { value: 'buy-now', label: '立刻入手' },
        { value: 'wait-for-better', label: '继续观望' },
        { value: 'upgrade-while-discounted', label: '升级换购' },
        { value: 'keep-or-buy-used', label: '保持现状或买二手' }
    ],
    storageKey: 'car_price_cut_choice.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=car-price-cut-choice'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>面对车企降价你会怎么选？结果</h2>';
