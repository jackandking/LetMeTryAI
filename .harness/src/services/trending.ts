...
async function fetchBaiduTrending(): Promise<TrendingItem[]> {
  try {
    ...
    console.warn('[Trending] Baidu fetch failed:', (e as Error).message);
    return [];
  }
}
...
async function fetchToutiaoTrending(): Promise<TrendingItem[]> {
  try {
    ...
    const json = await res.json();
    return (json.data || []).slice(0, 15).map((item: { Title?: string; title?: string; HotValue?: string }) => ({
      title: item.Title || item.title || '',
      hot: item.HotValue || ''
    })).filter((i: TrendingItem) => i.title);
  } catch (e) {
    console.warn('[Trending] Toutiao fetch failed:', (e as Error).message);
    return [];
  }
}

const PLATFORM_FETCHERS: Record<string, PlatformConfig> = {
...
export async function fetchTrendingTopics(options: TrendingOptions = {}): Promise<string> {
