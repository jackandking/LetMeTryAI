...
import { VideoRecorder, generateVoteAppVideoConfig, type VideoRecorderResult } from '../tools/video-recorder.js';
...
export class VideoGeneratorService {
  ...
  private async recordVideo(
    ...
    const videoConfig = generateVoteAppVideoConfig(
      config.appUrl,
      videoPath,
      config.appName
    );
   ...
   * 生成视频元数据
   */
  private generateMetadata(config: VideoGenerationConfig): {
    title: string;
    description: string;
    tags: string[];
  } {
   ...
   * 生成默认标题
   */
  private generateDefaultTitle(appName: string): string {
    const templates = [
      `快来试试${appName}，投出你的一票！`,
      `${appName} - 你的选择很重要`,
      `发现有趣的${appName}，一起来投票`,
   ...
   * 生成默认描述
   */
  private generateDefaultDescription(appName: string): string {
   ...
   * 生成默认标签
   */
  private generateDefaultTags(appName: string): string[] {
 ...
 * 便捷函数：生成应用演示视频
 */
export async function generateAppDemoVideo(
  appId: string,
  appName: string,
  appUrl: string,
  outputDir: string
 ...
 * 加载视频任务
 */
export async function loadVideoTask(
  taskId: string,
  tasksDir: string
  ...
  try {
    ...
    return JSON.parse(content) as VideoTask;
  } catch {
    return null;
  }
}
