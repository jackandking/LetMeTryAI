 ...
 * 从应用页面提取内容
 */
async function extractAppContent(page: Page): Promise<AppContent> {
...
): Promise<{ success: boolean; content?: AppContent; error?: string }> {
 ...
 * 生成配音脚本
 */
function generateScript(content: AppContent, appName: string): string {
...
async function generateAudio(text: string, outputPath: string): Promise<number> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', async (code) => {
      ...
      // 清理临时文件
      try { await fs.unlink(aiffPath); } catch {}
 ...
 * 主函数：生成带截图的视频
 */
export async function generateScreenshotVideo(
  config: ScreenshotVideoConfig
): Promise<ScreenshotVideoResult> {
 ...
 * 便捷函数
 */
export async function generateVideoFromScreenshot(
  appId: string,
  appName: string,
  appUrl: string,
  outputDir: string
