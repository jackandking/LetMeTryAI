...
export class VideoRecorder {
   ...
   * 录制应用演示视频
   */
  async record(config: VideoRecorderConfig): Promise<VideoRecorderResult> {
   ...
   * 启动浏览器并准备录制
   */
  private async launchBrowser(config: VideoRecorderConfig): Promise<void> {
   ...
   * 执行交互步骤
   */
  private async executeSteps(config: VideoRecorderConfig): Promise<void> {
   ...
   * 执行投票步骤
   */
  private async executeVoteStep(step: InteractionStep): Promise<void> {
    if (!this.page) return;
   ...
   * 生成默认的交互步骤（投票应用演示）
   */
  private generateDefaultSteps(): InteractionStep[] {
    return [
   ...
   * 关闭浏览器并保存视频
   */
  private async closeAndSave(): Promise<string> {
    if (!this.context) throw new Error('Context not initialized');

    const video = this.page?.video();
    if (!video) throw new Error('Video recording not available');
 ...
 * 录制视频（便捷函数）
 */
export async function recordAppVideo(
  appUrl: string,
  outputPath: string,
  options?: Omit<VideoRecorderConfig, 'appUrl' | 'outputPath'>
 ...
 * 生成投票应用演示视频配置
 */
export function generateVoteAppVideoConfig(
  appUrl: string,
  outputPath: string,
  appName: string
): VideoRecorderConfig {
