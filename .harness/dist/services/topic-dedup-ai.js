 ...
 */
function buildDedupPrompt(check) {
 ...
 */
function buildBatchDedupPrompt(candidates, historicalTitles) {
 ...
 */
export async function checkDuplicateWithAI(candidateTitle, historicalTitles, candidateDescription) {
    ...
    try {
        ...
        logger.error('AI dedup error', error);
 ...
 */
export async function batchCheckDuplicatesWithAI(candidates, historicalTitles) {
    ...
    try {
        ...
        if (!result.success || !result.data) {
            ...
            return Promise.all(candidates.map(c => checkDuplicateWithAI(c.title, historicalTitles, c.description)));
        }
        const data = result.data;
        ...
        logger.error('Batch AI dedup error, falling back', error);
        // Fallback to individual checks
        return Promise.all(candidates.map(c => checkDuplicateWithAI(c.title, historicalTitles, c.description)));
    }
}
 ...
 */
export async function deduplicateCandidatesWithAI(candidates, historicalTitles) {
...
//# sourceMappingURL=topic-dedup-ai.js.map