...
export function buildTopicSelectionPrompt(profile, date, trendingContext, recentTopics, topicHint) {
...
export function parseTopicSelectionResponse(content) {
    ...
    const candidates = obj.topicCandidates.map((c, idx) => {
        ...
        // Normalize options
        const options = candidate.options.map((o, oidx) => {
            const opt = o;
            return {
 ...
 */
export async function generateTopicsWithAI(profile, date = new Date().toISOString().split('T')[0], trendingContext) {
    ...
    if (!result.success) {
        logger.error('AI topic generation failed', result.error);
    ...
    logger.info('Topic generation successful', {
        provider: content?._meta?.provider,
        candidateCount: parsed.topicCandidates.length,
    });
    return parsed;
}
export async function chooseBestTopic(candidates, profile, options = {}) {
    ...
    const { deduplicateCandidates, deduplicateCandidatesWithAI, loadPublishedTaskNames } = await import('./topic-dedup.js');
    ...
    logger.info('Selected topic', {
        ...
        wasBlocked: best._blocked || false,
    });
    return best;
}
/**
...
function scoreCandidate(candidate, profile) {
    ...
    const doMoreMatch = profile.topicGuidelines.doMore.some(guideline => {
        ...
        return keywords.some(kw => candidate.title.toLowerCase().includes(kw) ||
...
//# sourceMappingURL=topic-selector.js.map