 ...
 * 使用 say 生成音频
 */
async function generateAudio(text, outputPath, voice = 'Ting-Ting', rate = 180) {
 ...
 * 下载或获取应用封面图
 */
async function getBackgroundImage(appId, outputDir) {
    // 尝试从应用目录获取封面图
    const possiblePaths = [
 ...
 * 使用 ffmpeg 合成视频
 */
async function composeVideo(imagePath, audioPath, outputPath, audioDuration) {
    return new Promise((resolve) => {
 ...
 * 生成 ffmpeg 合成脚本 (备用方案)
 */
function generateFfmpegScript(imagePath, audioPath, outputPath, scriptPath) {
    const videoInput = imagePath
        ? `-loop 1 -i "${imagePath}"`
 ...
 * 生成配音脚本
 */
function generateScript(appName, question, options) {
 ...
 * 视频生成服务 (say + ffmpeg)
 */
export async function generateSayFfmpegVideo(config) {
    const startTime = Date.now();
    try {
        await fs.mkdir(config.outputDir, { recursive: true });
        // 1. 生成配音文本
        const script = config.script || generateScript(config.appName, '主战坦克：火力、装甲、机动哪个影响实际表现？', ['火力优先（强打击）', '装甲优先（防护强）', '机动优先（快速机动）']);
        // 保存脚本文本
        ...
        const audioResult = await generateAudio(script, audioPath, config.voice, config.rate);
        ...
        // 4. 检查 ffmpeg 是否可用
        const ffmpegPath = getFfmpegPath();
        const ffmpegAvailable = ffmpegPath !== 'ffmpeg' || await new Promise((resolve) => {
        ...
        if (ffmpegAvailable && audioResult.duration) {
            ...
            const composeResult = await composeVideo(imagePath, audioPath, videoPath, audioResult.duration);
 ...
 * 便捷函数
 */
export async function generateVideoWithVoice(appId, appName, appUrl, outputDir, customScript) {
...
//# sourceMappingURL=video-say-ffmpeg.js.map