...
import { checkDuplicateWithAI, batchCheckDuplicatesWithAI } from './topic-dedup-ai.js';
 ...
 */
export function checkKeywordDuplicate(newTopic, recentTopics, profileId) {
 ...
 */
export async function checkTopicDuplicateWithAI(newTopic, recentTopics, options = {}) {
 ...
 */
export function loadPublishedTaskNames() {
    ...
    try {
        ...
        const latestReport = JSON.parse(readFileSync(join(metricsDir, files[0]), 'utf-8'));
 ...
 */
export function deduplicateCandidates(candidates, options) {
    ...
    return candidates.map(candidate => {
        ...
        const publishedCheck = checkTopicDuplicate(candidate.title, publishedNames, { threshold, profileId });
        ...
        for (const blockedId of blockedAppIds) {
            const selfCheck = checkTopicDuplicate(candidate.title, [blockedId], { threshold: 0.8, profileId } // Stricter for same-batch
            );
 ...
 */
export async function deduplicateCandidatesWithAI(candidates, options) {
    ...
    const aiResults = await batchCheckDuplicatesWithAI(needAiCheck.map(c => ({ title: c.title, description: c.description })), publishedNames);
    // Merge results
    const aiResultMap = new Map(needAiCheck.map((c, i) => [c.title, aiResults[i]]));
    return jaccardResults.map(c => {
        if (c._blocked)
            return c; // Already blocked by keyword or Jaccard
...
//# sourceMappingURL=topic-dedup.js.map