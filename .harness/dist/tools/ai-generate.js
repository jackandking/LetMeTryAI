...
export const aiGenerateTool = {
    ...
    async execute(args) {
        ...
        const { prompt, model, outputFormat = 'text', temperature, fallbackOnTimeout = true, } = args;
        ...
        for (const provider of PROVIDERS.sort((a, b) => a.priority - b.priority)) {
            ...
            try {
                const result = await executeWithTimeout(provider.tool, {
                    prompt,
 ...
 */
async function executeWithTimeout(tool, args, timeoutMs) {
