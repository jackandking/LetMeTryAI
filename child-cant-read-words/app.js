...
const questionConfig = {
    title: '孩子拿着书却认不出字，你会先怎么做？',
    question: '孩子拿着书却认不出字，你会先怎么做？',
    options: [
        { value: 'point-and-read', label: '边指边教，跟读示范' },
        { value: 'vision-development-check', label: '先约视力/发育筛查' },
        { value: 'picture-story-guided', label: '用图画故事吸引兴趣' },
        { value: 'playful-literacy-app', label: '换成识字游戏或趣味App' }
    ],
    storageKey: 'child_cant_read_words.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=child-cant-read-words'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>孩子拿着书却认不出字，你会先怎么做？结果</h2>';
