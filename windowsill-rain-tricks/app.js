...
const questionConfig = {
    title: '下雨把窗台灌水，你会先用哪招处理？老办法还是新花样',
    question: '下雨把窗台灌水，你会先用哪招处理？',
    options: [
        { value: 'wipe-with-cloth', label: '用抹布或拖把擦干' },
        { value: 'scoop-into-basin', label: '用盆/杯舀水倒掉' },
        { value: 'towel-newspaper-absorb', label: '毛巾＋旧报纸吸干缝隙' },
        { value: 'open-window-basin', label: '打开窗放盆接水并临时堵缝' }
    ],
    storageKey: 'windowsill_rain_tricks.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=windowsill-rain-tricks'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>下雨把窗台灌水，你会先用哪招处理？老办法还是新花样结果</h2>';
