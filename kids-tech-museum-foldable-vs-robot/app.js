...
const questionConfig = {
    title: '带孩子逛科技馆，先看折叠手机展还是机器人互动？',
    question: '带孩子逛科技馆，先看折叠手机展还是机器人互动？',
    options: [
        { value: 'foldable-phone-first', label: '先看折叠手机展（静态观察）' },
        { value: 'robot-interaction-first', label: '先去机器人互动区（动手体验）' },
        { value: 'follow-child-interest', label: '按孩子兴趣随性走（跟着TA）' },
        { value: 'quick-scan-then-return', label: '先都快速看一圈再回访喜欢的展区' }
    ],
    storageKey: 'kids_tech_museum_foldable_vs_robot.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=kids-tech-museum-foldable-vs-robot'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>带孩子逛科技馆，先看折叠手机展还是机器人互动？结果</h2>';
