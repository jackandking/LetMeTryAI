...
const questionConfig = {
    title: '电动车续航与费用：你更在意哪点？',
    question: '在购车或换车时，你更在意电动车的续航里程还是长期使用费用？请选择符合你看法的选项。',
    options: [
        { value: 'range-priority', label: '续航里程' },
        { value: 'cost-priority', label: '使用成本' },
        { value: 'charging-convenience', label: '充电便利' },
        { value: 'performance-resale', label: '性能与保值' }
    ],
    storageKey: 'electric_range_cost.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=electric-range-cost'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>电动车续航与费用：你更在意哪点？结果</h2>';
