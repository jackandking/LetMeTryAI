/**
 * {{TITLE}} - Voting Survey Application
 * Logic for the "{{TITLE}}" survey
 */
...
const EVENT_ENDPOINT = '{{EVENT_ENDPOINT}}';
...
function attachRadioHandlers() {
    const radios = document.querySelectorAll('input[name="{{INPUT_NAME}}"]');
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id={{APP_ID}}'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = "<h2 style='text-align:center;color:#2f4858;'>{{RESULT_HEADING}}</h2>";
    resultDiv.innerHTML += "<p style='text-align:center;color:#7f8c8d;margin-bottom:20px;font-size:14px;'>{{RESULT_SUBTITLE}}</p>";
