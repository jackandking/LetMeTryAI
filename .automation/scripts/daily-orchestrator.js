...
export function buildTopicSelectionPrompt({ profile, currentDate, recentTopics, trendingContext, topicHint }) {
    ...
    return [
        ...
        trendingContext ? `\n${trendingContext}\n` : '',
        trendingContext ? `\n热搜→投票转化指南（重要，必须遵守）：\n1. 不要直接复制热搜事件本身（如政治新闻、犯罪事件、社会争议不适合做投票）。\n2. 要从热搜中读取：人们在关心什么品类、生活方式、情感话题或消费趋势？\n3. 把"事件"转化为"互动投票"——找到事件中天然适合站队、预测、选择的切入点：
   - 体育赛事类 → 预测/站队：热搜"国乒女团晋级决赛" → 投票"决赛预测：中国 vs 日本，你更看好谁夺冠？"
   - 消费趋势类 → 品类PK：热搜"榴莲价格跌至1字头" → 投票"榴莲品种争霸：你最爱哪一种？"
   - 生活方式类 → 习惯调查：热搜"加速衰老的5大行为" → 投票"这些习惯你占几个？哪个最该戒掉？"
   - 美食热点类 → 口味选择：热搜"小伙西装夜市摆摊月入5万" → 投票"夜市美食争霸：你最馋哪一口？"
   - 社会话题类 → 观点站队：热搜"家长控诉Cosplay毁掉孩子" → 投票"如果孩子迷上二次元，你会支持吗？"
   - 娱乐新闻类 → 颜值/造型PK：热搜"刘浩存现场表演" → 投票"红毯造型PK：谁的look最惊艳？"
   - 科技产品类 → 功能/外观选择：热搜"某手机芯片厂漏洞" → 投票"手机功能大比拼：你最看重哪一项？"
