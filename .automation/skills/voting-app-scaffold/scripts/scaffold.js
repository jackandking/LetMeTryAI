...
export function renderOptionMarkup(options, inputName = 'fighter') {
    ...
    return normalized
        .map(option => [
            ...
            `                                <h3 class="option-title">${escapeHtml(option.label)}</h3>`,
            `                                <p class="option-desc">${escapeHtml(option.caption)}</p>`,
            '                            </div>',
...
export function renderFullIndexHtml(spec) {
    ...
    return [
        ...
        `                    <p id="questionText" class="question-text">${escapeHtml(question)}</p>`,
        ...
        '                <a target="_blank" onclick="jumpToIndex()">返回主页查看更多</a>',
        '            </div>',
        '        </footer>',
        '    </div>',
        '',
