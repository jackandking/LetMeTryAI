...
const questionConfig = {
    title: '总犯困、代谢慢？真相是哪种日常习惯在作怪？',
    question: '你觉得导致总犯困、代谢慢的主要日常习惯是哪种？',
    options: [
        { value: 'irregular-sleep', label: '作息零散（晚睡/午睡不稳）' },
        { value: 'sedentary-lifestyle', label: '久坐少动/不主动活动' },
        { value: 'late-heavy-meals', label: '饮食重油重糖/吃太晚' },
        { value: 'low-water-intake', label: '喝水少/忽视日常补水' }
    ],
    storageKey: 'fatigue_metabolism_habits.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=fatigue-metabolism-habits'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>总犯困、代谢慢？真相是哪种日常习惯在作怪？结果</h2>';
