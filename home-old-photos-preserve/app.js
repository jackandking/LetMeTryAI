...
const questionConfig = {
    title: '家中旧影像与老照片，你会如何处理或珍藏？',
    question: '家中旧影像与老照片，你通常会如何处理或珍藏？',
    options: [
        { value: 'digitize-and-backup', label: '数字化备份' },
        { value: 'album-and-frame-display', label: '手工相册与相框展示' },
        { value: 'pass-to-family', label: '传给家人保存' },
        { value: 'creative-reuse-collage', label: '创意再利用（拼贴/影集/礼物）' }
    ],
    storageKey: 'home_old_photos_preserve.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=home-old-photos-preserve'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家中旧影像与老照片，你会如何处理或珍藏？结果</h2>';
