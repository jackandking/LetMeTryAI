...
const questionConfig = {
    title: '雪糕里咬到异物你会怎么办？你会怎么反应',
    question: '雪糕里咬到异物你会怎么办？选一个符合你的做法',
    options: [
        { value: 'photo-complain-return', label: '拍照投诉并退货' },
        { value: 'spit-and-refuse', label: '当场吐出并停吃' },
        { value: 'post-and-warn', label: '拍照发圈提醒朋友' },
        { value: 'discard-and-ignore', label: '直接丢掉不再理' }
    ],
    storageKey: 'ice_cream_foreign_object.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=ice-cream-foreign-object'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>雪糕里咬到异物你会怎么办？你会怎么反应结果</h2>';
