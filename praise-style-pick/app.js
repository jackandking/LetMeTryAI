/**
 * 表扬方式投票 Survey Application
 * Logic for the "家长话题：表扬方式哪种更有效" survey
 */

const questionConfig = {
    "title": "家长话题：表扬方式哪种更有效",
    "question": "站在家长视角，你会怎么选：表扬孩子时更常用哪种方式？",
    "options": [
        {
            "value": "praise-specific-behavior",
            "label": "具体行为表扬（说出做得好的地方）"
        },
        {
            "value": "praise-effort-process",
            "label": "过程与努力表扬（强调努力和方法）"
        },
        {
            "value": "praise-character-trait",
            "label": "性格/天赋表扬（夸孩子“聪明”或“乖”）"
        },
        {
            "value": "praise-physical-affection",
            "label": "适度肢体鼓励（拥抱、击掌等）"
        }
    ],
    "storageKey": "praise_style_pick_v1.data"
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=praise-style-pick'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = "<h2 style='text-align:center;color:#2f4858;'>表扬方式投票投票结果</h2>";
    resultDiv.innerHTML += "<p style='text-align:center;color:#7f8c8d;margin-bottom:20px;font-size:14px;'>看看大家对“家长话题：表扬方式哪种更有效”的最新态度</p>";
