...
const questionConfig = {
    title: '家里还用空气炸锅纸吗？实测用法和坑揭秘',
    question: '你家还用空气炸锅纸吗？哪种做法更像你家的习惯？',
    options: [
        { value: 'always-use-paper', label: '一直铺纸' },
        { value: 'no-paper-spray', label: '不用纸，喷油或刷油' },
        { value: 'use-for-sticky-foods', label: '只在易黏食材时用' },
        { value: 'use-silicone-mat', label: '换用硅胶垫重复使用' }
    ],
    storageKey: 'airfryer_paper_usage.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=airfryer-paper-usage'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家里还用空气炸锅纸吗？实测用法和坑揭秘结果</h2>';
