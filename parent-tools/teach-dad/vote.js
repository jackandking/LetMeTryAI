/**
 * Video Voting Application for Teach Dad
 */
        ...
        if (data) {
            try {
                ...
                if (allDances.length < voteConfig.numberOfDishes) {
                    showError(`视频数量不足，至少需要${voteConfig.numberOfDishes}条`);
                    return;
                }
            ...
            showError('暂无视频数据，请先在教爸爸带男娃页面添加视频');
        }
    });
}
...
function showAd() {
    if (selectedDanceIndex === null) { alert('请先选择一条视频！'); return; }
    saveVote();
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: "/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=parent-tools/teach-dad",
        });
    } else {
