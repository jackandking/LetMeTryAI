...
const questionConfig = {
    title: '新片动作对比：谁更带感',
    question: '你觉得哪类动作片更带感？',
    options: [
        { value: 'guochan-action', label: '国产动作片' },
        { value: 'hollywood-action', label: '好莱坞动作片' },
        { value: 'indie-action', label: '独立/风格化动作片' }
    ],
    storageKey: 'action_films_faceoff.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=action-films-faceoff'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>新片动作对比：谁更带感结果</h2>';
