...
const questionConfig = {
    title: "男人减速带：哪个最让你挪不动步？",
    question: "除了钓鱼和充电头，以下哪种画面能让你瞬间驻足，一看就是半小时？",
    options: [
        { value: "excavator", label: "挖掘机 (基建狂魔)" },
        { value: "forging", label: "锻刀 (淬火瞬间)" },
        { value: "cnc", label: "数控机床 (削铁如泥)" },
        { value: "dam", label: "大坝泄洪 (雷霆万钧)" },
        { value: "welding", label: "鱼鳞焊 (强迫症福音)" }
    ],
    storageKey: "mans_speed_bump_v1.data"
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: "/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=mans-speed-bump",
        });
        return;
    }
...
function showResult(voteData) {
    ...
    resultDiv.innerHTML = "<h2 style='text-align:center;color:#2c3e50;'>减速带投票结果</h2>";
    resultDiv.innerHTML += "<p style='text-align:center;color:#7f8c8d;margin-bottom:20px;font-size:14px;'>基于实时全网猛男统计</p>";
...
function createBarChart(voteData) {
    ...
    for (const [option, count] of sortedEntries) {
        ...
        optionLabel.innerText = option.split(' ')[0]; // Just show "挖掘机", "锻刀" etc
...
function addSummaryStatistics(container, voteData) {
    ...
    totalVotes.innerText = `总投票人数: ${total}`;
