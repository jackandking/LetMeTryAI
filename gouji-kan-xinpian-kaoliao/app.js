...
const questionConfig = {
    title: '购机看芯片的考量：性能、续航还是生态？',
    question: '购机看芯片的考量：你在挑选手机时更看重哪项？',
    options: [
        { value: 'cpu-gpu-performance', label: '性能优先（CPU/GPU）' },
        { value: 'battery-thermal', label: '续航与发热控制' },
        { value: 'camera-media', label: '拍照与多媒体体验' },
        { value: 'ecosystem-updates', label: '系统生态与更新支持' }
    ],
    storageKey: 'gouji_kan_xinpian_kaoliao.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=gouji-kan-xinpian-kaoliao'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>购机看芯片的考量：性能、续航还是生态？结果</h2>';
