/**
 * 球星对决 Survey Application
 * Logic for the "男人宝热榜：谁是当下最具统治力的现役球星？" survey
 */

const questionConfig = {
    "title": "男人宝热榜：谁是当下最具统治力的现役球星？",
    "question": "站在男人宝用户视角，你更想把票投给谁：谁是当下最具统治力的现役球星？",
    "options": [
        {
            "value": "lionel-messi",
            "label": "Lionel Messi"
    ...
    "storageKey": "top_active_football_stars_v1.data"
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=top-active-football-stars'
        });
        return;
    }
...
function displayAdFallback() {
    return new Promise((resolve) => {
        ...
        if (!overlay) {
            ...
            overlay.innerHTML = '<div style="background:#2c5aa0;padding:30px;border-radius:12px;text-align:center;box-shadow:0 10px 25px rgba(0,0,0,0.5);"><h3>正在分析"男人宝热榜：谁是当下最具统治力的现役球星？"的投票趋势...</h3><div style="margin-top:15px;width:40px;height:40px;border:4px solid #4a7ab8;border-top:4px solid transparent;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto;"></div><style>@keyframes spin {0% {transform: rotate(0deg);} 100% {transform: rotate(360deg);}}</style></div>';
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = "<h2 style='text-align:center;color:#2c5aa0;'>球星对决投票结果</h2>";
    resultDiv.innerHTML += "<p style='text-align:center;color:#5a6a7a;margin-bottom:20px;font-size:14px;'>看看大家对"男人宝热榜：谁是当下最具统治力的现役球星？"的最新态度</p>";
...
function addSummaryStatistics(container, latestVoteData) {
    ...
    totalVotes.style.cssText = 'font-weight: bold; color: #2c5aa0;';
