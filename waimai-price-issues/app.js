...
const questionConfig = {
    title: '外卖涨价后你遇到过哪些糟心事？',
    question: '外卖涨价后，哪种糟心情况你经常遇到？',
    options: [
        { value: 'late-delivery', label: '配送慢或迟到' },
        { value: 'missing-items', label: '缺少菜品或少份量' },
        { value: 'extra-fee', label: '出现隐形收费' },
        { value: 'wrong-item', label: '送错或口味与描述不符' }
    ],
    storageKey: 'waimai_price_issues.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=waimai-price-issues'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>外卖涨价后你遇到过哪些糟心事？结果</h2>';
