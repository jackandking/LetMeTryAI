...
const questionConfig = {
    title: '你会优先尝试哪类美妆护肤趋势？选出你下一个要试的护肤方向',
    question: '你会优先尝试哪类美妆护肤趋势？',
    options: [
        { value: 'hyaluronic-hydration', label: '玻尿酸补水' },
        { value: 'ceramide-barrier-repair', label: '屏障修护（神经酰胺）' },
        { value: 'skinimalism-light-makeup', label: '轻薄裸妆（skinimalism）' },
        { value: 'active-serum-essence', label: '功效型精华（维C/视黄醇）' }
    ],
    storageKey: 'womanai_makeup_trends_vote.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=womanai-makeup-trends-vote'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>你会优先尝试哪类美妆护肤趋势？选出你下一个要试的护肤方向结果</h2>';
