...
const questionConfig = {
    title: '高速上见到驾驶疑似打盹，你会怎么应对？',
    question: '高速上见到驾驶疑似打盹，你会怎么应对？',
    options: [
        { value: 'wake-and-suggest-replace-driver', label: '提醒并建议靠边休息或换人' },
        { value: 'keep-distance-call-for-help', label: '保持距离并联系路政或报警' },
        { value: 'pull-over-and-stop', label: '靠边记录并等待安全后上前协助' },
        { value: 'avoid-and-report', label: '不直接接近，避让并事后上报' }
    ],
    storageKey: 'gaosu_driver_drowsy_response.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=gaosu-driver-drowsy-response'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>高速上见到驾驶疑似打盹，你会怎么应对？结果</h2>';
