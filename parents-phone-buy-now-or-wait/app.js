...
const questionConfig = {
    title: '爸妈换手机：现在买还是等等？',
    question: '你会怎么决定：爸妈现在换手机，还是先等等/不换？',
    options: [
        { value: 'buy-now-big-screen-simple', label: '现在换：买大屏简机' },
        { value: 'buy-now-smart-preconfig', label: '现在换：买智能并预设功能' },
        { value: 'wait-for-better-model-or-sale', label: '等等：等更懂老人的机型或降价' },
        { value: 'keep-current-teach-and-optimize', label: '先不换：教会用并优化当前手机' }
    ],
    storageKey: 'parents_phone_buy_now_or_wait.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=parents-phone-buy-now-or-wait'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>爸妈换手机：现在买还是等等？结果</h2>';
