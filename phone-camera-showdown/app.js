...
const questionConfig = {
    title: '智能手机拍照偏好投票',
    question: '你在手机拍照时最看重哪类表现？',
    options: [
        { value: 'low-light-performance', label: '夜景表现' },
        { value: 'portrait-bokeh-skin', label: '人像虚化与肤色' },
        { value: 'wide-angle-detail', label: '超广角与细节' }
    ],
    storageKey: 'phone_camera_showdown.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=phone-camera-showdown'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>智能手机拍照偏好投票结果</h2>';
