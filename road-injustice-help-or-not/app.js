...
const questionConfig = {
    title: '路见不平，你会主动出手吗？',
    question: '路见不平，你会主动出手吗？',
    options: [
        { value: 'step-forward', label: '挺身而出' },
        { value: 'record-evidence', label: '先留证据' },
        { value: 'assess-situation', label: '看情况帮' },
        { value: 'stay-safe', label: '明哲保身' }
    ],
    storageKey: 'road_injustice_help_or_not.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=road-injustice-help-or-not'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>路见不平，你会主动出手吗？结果</h2>';
