...
const questionConfig = {
    title: '老师写下千字辞职信，教室里哪个画面会先浮现？',
    question: '老师写下千字辞职信，教室里哪个画面会先浮现？',
    options: [
        { value: 'blackboard-farewell', label: '黑板告别字' },
        { value: 'students-teary', label: '学生泪目瞬间' },
        { value: 'podium-photo-award', label: '讲台的老照片' },
        { value: 'desk-drawer-letter', label: '课桌抽屉的信' }
    ],
    storageKey: 'teacher_resign_classroom_scene.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=teacher-resign-classroom-scene'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>老师写下千字辞职信，教室里哪个画面会先浮现？结果</h2>';
