...
const questionConfig = {
    title: '哪些日常习惯跟心梗脑梗有关？你平时注意到哪些',
    question: '哪些日常习惯跟心梗脑梗有关？在下面场景中选出你最担心或家里人常有的那些（可多选）',
    options: [
        { value: 'jusu-shao-dong', label: '久坐少动（整天坐着看电视或不爱活动）' },
        { value: 'zhong-kouwei-yinshi', label: '重口味饮食（咸、油、外卖多）' },
        { value: 'aoye-ya-li', label: '熬夜少眠与长期精神压力' },
        { value: 'xiyan-xujiu', label: '吸烟或过量饮酒（社交或独自饮酒）' }
    ],
    storageKey: 'habits_heart_stroke.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=habits-heart-stroke'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>哪些日常习惯跟心梗脑梗有关？你平时注意到哪些结果</h2>';
