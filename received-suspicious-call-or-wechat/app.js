...
const questionConfig = {
    title: '收到可疑电话或微信时您会怎么处理',
    question: '当您收到可疑的电话或微信时，通常会怎么办？',
    options: [
        { value: 'hang-up-and-block', label: '直接挂断并拉黑' },
        { value: 'verify-with-family', label: '先联系家人或信任的人核实' },
        { value: 'call-helpline-or-community', label: '求助防骗热线或社区帮助' },
        { value: 'keep-record-and-inform', label: '不回复但保存信息并提醒亲友' }
    ],
    storageKey: 'received_suspicious_call_or_wechat.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=received-suspicious-call-or-wechat'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>收到可疑电话或微信时您会怎么处理结果</h2>';
