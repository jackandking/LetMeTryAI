...
const questionConfig = {
    title: '家庭资金优先用在哪儿？公积金用途变化下的选择',
    question: '在公积金用途变化的背景下，您会把家庭资金优先用在哪个方向？',
    options: [
        { value: 'mortgage-repayment', label: '优先偿还房贷' },
        { value: 'education-savings', label: '优先教育储蓄' },
        { value: 'emergency-fund', label: '优先家庭应急与保障' },
        { value: 'home-improvement', label: '优先改善家庭生活' }
    ],
    storageKey: 'family_fund_priority.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=family-fund-priority'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家庭资金优先用在哪儿？公积金用途变化下的选择结果</h2>';
