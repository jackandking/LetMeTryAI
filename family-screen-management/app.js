/**
 * 孩子手机使用管理 Survey Application
 * Logic for the "家长话题：孩子手机使用时间怎么管？" survey
 */

const questionConfig = {
    "title": "家长话题：孩子手机使用时间怎么管？",
    "question": "站在家长视角，你会怎么选：孩子手机使用时间怎么管？",
    "options": [
        {
            "value": "fixed-daily-limit",
            "label": "固定每日时长（每日限定X小时）"
        },
        {
            "value": "time-slot-use",
            "label": "分时段开放（作业后/周末）"
        ...
        {
            "value": "reward-based-time",
            "label": "表现兑换时长（奖惩式）"
        }
    ],
    "storageKey": "family_screen_management_v1.data"
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=family-screen-management'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = "<h2 style='text-align:center;color:#2f4858;'>孩子手机使用管理投票结果</h2>";
    resultDiv.innerHTML += "<p style='text-align:center;color:#7f8c8d;margin-bottom:20px;font-size:14px;'>看看大家对“家长话题：孩子手机使用时间怎么管？”的最新态度</p>";
