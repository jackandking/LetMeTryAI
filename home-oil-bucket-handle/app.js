...
const questionConfig = {
    title: '家里开了桶食用油，你会怎么处理？分装、放阳台还是优先用？',
    question: '家里开了桶食用油，你会怎么处理？选出最常做的那种。',
    options: [
        { value: 'fen-zhuang-xiao-ping', label: '分装小瓶，方便取用' },
        { value: 'yuan-tong-mi-feng', label: '原桶密封，放阴凉避光处' },
        { value: 'you-xian-yong-wan', label: '优先用完，做菜时先用这桶' },
        { value: 'fen-gei-jia-ren', label: '分给家人或邻居一起用' }
    ],
    storageKey: 'home_oil_bucket_handle.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=home-oil-bucket-handle'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家里开了桶食用油，你会怎么处理？分装、放阳台还是优先用？结果</h2>';
