...
const questionConfig = {
    title: '《浪姐》二公舞台造型投票：你喜欢哪个舞台造型？',
    question: '你喜欢《浪姐》二公舞台哪个造型？',
    options: [
        { value: 'elegant-gown', label: '华丽礼服（长裙）' },
        { value: 'sharp-androgynous', label: '利落中性（短装）' },
        { value: 'retro-elegance', label: '复古优雅（复古风）' },
        { value: 'minimal-modern', label: '简约现代（低调摩登）' }
    ],
    storageKey: 'langjie_ergong_stage_style_vote.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=langjie-ergong-stage-style-vote'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>《浪姐》二公舞台造型投票：你喜欢哪个舞台造型？结果</h2>';
