...
const questionConfig = {
    title: '主战坦克对比投票',
    question: '在主战坦克的设计上，你最看重哪一项性能？',
    options: [
        { value: 'firepower', label: '火力优先（射控与口径）' },
        { value: 'protection', label: '防护优先（装甲与主动防护）' },
        { value: 'mobility', label: '机动优先（悬挂与发动机）' }
    ],
    storageKey: 'main_battle_tank_compare.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=main-battle-tank-compare'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>主战坦克对比投票结果</h2>';
