...
const questionConfig = {
    title: '电动跑车续航与操控对比',
    question: '买电动跑车你最看重哪项表现？',
    options: [
        { value: 'long-range-gt', label: '长续航GT' },
        { value: 'track-focused-sports-car', label: '赛道操控型' },
        { value: 'balanced-performance-coupe', label: '均衡性能跑车' },
        { value: 'fast-charging-priority', label: '充电便利优先型' }
    ],
    storageKey: 'electric_sports_car_showdown.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=electric-sports-car-showdown'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>电动跑车续航与操控对比结果</h2>';
