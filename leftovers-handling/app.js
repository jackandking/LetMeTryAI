...
const questionConfig = {
    title: '家里的剩菜剩饭，你都会怎么处理？留着吃、改造还是扔掉？',
    question: '家里的剩菜剩饭，你都会怎么处理？',
    options: [
        { value: 'keep-in-fridge', label: '放冰箱留着再吃' },
        { value: 'eat-same-day', label: '当晚吃完不留' },
        { value: 'repurpose-into-new-dish', label: '改造成新菜' },
        { value: 'freeze-for-later', label: '分装冷冻保存' }
    ],
    storageKey: 'leftovers_handling.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=leftovers-handling'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>家里的剩菜剩饭，你都会怎么处理？留着吃、改造还是扔掉？结果</h2>';
