/**
 * AI Lobster Survey Application
 * Logic for the "AI养龙虾，你买不买单？" survey
 */

const questionConfig = {
    title: 'AI养龙虾，你买不买单？',
    question: '面对今天刷屏的“AI养龙虾”热点，你最认可哪种态度？',
    options: [
        { value: 'yield', label: '智能提产派 AI增产' },
        { value: 'taste', label: '口感优先派 鲜味第一' },
        { value: 'price', label: '只看价格派 便宜就冲' },
        { value: 'future', label: '科技尝鲜派 先试再说' },
        { value: 'skeptic', label: '智商税警觉派 先别吹' }
    ],
    storageKey: 'ai_lobster_v1.data'
};
...
function attachRadioHandlers() {
    const radios = document.querySelectorAll('input[name="lobster"]');
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=ai-lobster'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = "<h2 style='text-align:center;color:#2f4858;'>AI龙虾态度投票结果</h2>";
    resultDiv.innerHTML += "<p style='text-align:center;color:#7f8c8d;margin-bottom:20px;font-size:14px;'>看看大家是为科技买单，还是为味道和价格站队</p>";
