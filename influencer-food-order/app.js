...
const questionConfig = {
    title: '你会为网红美食下单尝鲜吗？来投票说说你的选择',
    question: '看到网红推荐的美食，你会为网红美食下单尝鲜吗？',
    options: [
        { value: 'order-now', label: '立刻下单尝鲜' },
        { value: 'wait-for-reviews', label: '先看评价再决定' },
        { value: 'try-once', label: '尝试一次看口味' },
        { value: 'never-order', label: '不会下单/不感兴趣' }
    ],
    storageKey: 'influencer_food_order.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=influencer-food-order'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>你会为网红美食下单尝鲜吗？来投票说说你的选择结果</h2>';
