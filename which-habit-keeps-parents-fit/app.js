...
const questionConfig = {
    title: '想不发福，爸妈更靠哪个日常习惯?',
    question: '想不发福，爸妈更靠哪个日常习惯?',
    options: [
        { value: 'light-dinner', label: '晚饭清淡少量' },
        { value: 'daily-walking', label: '每天饭后散步' },
        { value: 'regular-housework', label: '常做家务动起来' },
        { value: 'cut-snacks', label: '控制零食甜点' }
    ],
    storageKey: 'which_habit_keeps_parents_fit.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=which-habit-keeps-parents-fit'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>想不发福，爸妈更靠哪个日常习惯?结果</h2>';
