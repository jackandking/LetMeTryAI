...
const questionConfig = {
    title: '长者出行：电动车与传统方式，你倾向哪种？',
    question: '长者出行：电动车与传统方式，你倾向哪种？',
    options: [
        { value: 'electric-mobility', label: '电动车代步' },
        { value: 'traditional-walking', label: '传统方式（步行/助行器/公交）' },
        { value: 'family-community-ride', label: '家庭陪同或社区接送' }
    ],
    storageKey: 'elder_transport_choice.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=elder-transport-choice'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>长者出行：电动车与传统方式，你倾向哪种？结果</h2>';
