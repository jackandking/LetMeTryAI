...
const questionConfig = {
    title: '孩子吃到减脂餐像被震惊，你会怎么处理这顿饭？',
    question: '孩子吃到减脂餐像被震惊，你会怎么处理这顿饭？',
    options: [
        { value: 'stop-and-replace', label: '当场制止并换成熟悉食物' },
        { value: 'soothe-then-dispose', label: '先安抚情绪，再悄悄处理剩菜' },
        { value: 'talk-and-explain', label: '当场用对话引导解释原因' },
        { value: 'observe-and-feedback', label: '先观察不强求，回家再反馈或记录' }
    ],
    storageKey: 'child_eats_diet_meal_shock.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=child-eats-diet-meal-shock'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>孩子吃到减脂餐像被震惊，你会怎么处理这顿饭？结果</h2>';
