 ...
 * Full-featured points system adapted from nanrenbao
 */
...
const PointsSystem = (function() {
    const STORAGE_KEYS = {
        ...
        VIEWED_IMAGES: 'womanai_viewed_images',
    ...
    /**
     * Mark an image as viewed without charging points (used for harem auto-collect)
    ...
    async function markImageViewed(imageUrl) {
        ...
        try {
            ...
            console.error('Failed to increment image view count:', err);
        }
    ...
    async function markImageDeletedInDB(imageUrl) {
        ...
        let response = await fetch(API_ENDPOINTS.MYSQL_QUERY, {
            ...
            body: JSON.stringify({
                sql: 'UPDATE handsome_images SET deleted = 1 WHERE image_url = ?',
                params: [imageUrl]
            })
        });
        ...
        if (Array.isArray(rows) && rows.length > 0) {
            ...
            const updResp = await fetch(API_ENDPOINTS.MYSQL_QUERY, {
                ...
                body: JSON.stringify({
                    sql: 'UPDATE handsome_images SET deleted = 1 WHERE id = ?',
                    params: [id]
                })
            });
