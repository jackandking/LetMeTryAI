// womanai/harem.js
// 我的后宫功能脚本 (adapted for womanai - 女人爱)

// 获取激活状态
function isHaremActivated() {
  return localStorage.getItem('womanaiHaremActivated') === '1';
}

// 激活后宫功能
function activateHarem() {
  localStorage.setItem('womanaiHaremActivated', '1');
}

// 获取后宫图片列表
function getHaremImages() {
  const imgs = localStorage.getItem('womanaiHaremImages');
...
function addHaremImage(url) {
  ...
  imgs.unshift(url);
  localStorage.setItem('womanaiHaremImages', JSON.stringify(imgs));
  return true;
}

// Capacity helpers
function getHaremCapacity() {
  const v = localStorage.getItem('womanaiHaremCapacity');
  return v ? parseInt(v, 10) : 0;
}

function setHaremCapacity(n) {
  localStorage.setItem('womanaiHaremCapacity', String(n));
}
