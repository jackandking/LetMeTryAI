...
const questionConfig = {
    title: '公积金还能这样用？你会把它给父母做什么用？',
    question: '公积金还能这样用？你会把它给父母做什么用？',
    options: [
        { value: 'help-parents-mortgage', label: '帮父母还房贷' },
        { value: 'upgrade-or-buy-home-for-parents', label: '支持换房或改善住房' },
        { value: 'home-safety-renovation', label: '做居家无障碍与安全改造' },
        { value: 'parent-retirement-reserve', label: '存作父母的养老备用金' }
    ],
    storageKey: 'gong_jijin_parents_use.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=gong-jijin-parents-use'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>公积金还能这样用？你会把它给父母做什么用？结果</h2>';
