...
const questionConfig = {
    title: '假期错峰出游：你更偏好哪种安排？',
    question: '假期错峰出游你更偏好哪种安排？',
    options: [
        { value: 'weekday-off-peak-short-trip', label: '周中错峰短途' },
        { value: 'early-morning-avoid-crowds', label: '清晨出发避开景点人流' },
        { value: 'late-holiday-departure', label: '假期尾段出发避开高峰' },
        { value: 'nearby-drive-flexible-itinerary', label: '近郊自驾与弹性行程' }
    ],
    storageKey: 'holiday_off_peak_travel_preference.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=holiday-off-peak-travel-preference'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>假期错峰出游：你更偏好哪种安排？结果</h2>';
