...
const questionConfig = {
    title: '孩子在公共牌匾或墙面涂鸦，你会怎样引导？',
    question: '如果发现孩子在公共牌匾或墙面涂鸦，你会选择怎样的引导方式？',
    options: [
        { value: 'talk-and-clean', label: '温和沟通并示范清理' },
        { value: 'set-limits-and-offer', label: '限制后允许创作替代' },
        { value: 'responsibility-and-apology', label: '引导承担责任并道歉' },
        { value: 'strict-consequence', label: '规则明确并收回部分特权' }
    ],
    storageKey: 'guide_child_graffiti.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=guide-child-graffiti'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>孩子在公共牌匾或墙面涂鸦，你会怎样引导？结果</h2>';
