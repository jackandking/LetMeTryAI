...
const questionConfig = {
    title: '苹果被罚会影响你的换机计划吗？',
    question: '苹果被罚会影响你的换机计划吗？',
    options: [
        { value: 'keep-planned', label: '照常换机' },
        { value: 'delay-and-watch', label: '暂缓观望' },
        { value: 'switch-brand', label: '改换其他品牌' },
        { value: 'wait-discount', label: '等促销折扣' }
    ],
    storageKey: 'apple_penalty_replace_plan.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=apple-penalty-replace-plan'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>苹果被罚会影响你的换机计划吗？结果</h2>';
