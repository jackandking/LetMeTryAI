...
const questionConfig = {
    title: '你会查看家人或伴侣的转账记录吗？',
    question: '你会查看家人或伴侣的转账记录吗？',
    options: [
        { value: 'check-occasionally', label: '会，偶尔核对' },
        { value: 'check-when-suspect', label: '会，必要时全面查看' },
        { value: 'respect-privacy', label: '不会，尊重隐私' },
        { value: 'talk-first', label: '不会，但会先沟通' }
    ],
    storageKey: 'family_transfer_privacy.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=family-transfer-privacy'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>你会查看家人或伴侣的转账记录吗？结果</h2>';
