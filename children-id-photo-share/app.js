...
const questionConfig = {
    title: '孩子证件照如何安全分享：亲友、学校与社交场景的选择',
    question: '在兼顾隐私与便利的前提下，你更倾向采用哪种方式分享孩子的证件照？',
    options: [
        { value: 'share-in-person', label: '线下当面或打印交付' },
        { value: 'share-private-link', label: '使用加密私有链接分享' },
        { value: 'share-anonymized', label: '去识别化后再上传或发送' },
        { value: 'share-school-only', label: '只通过学校或官方渠道提交' }
    ],
    storageKey: 'children_id_photo_share.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=children-id-photo-share'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>孩子证件照如何安全分享：亲友、学校与社交场景的选择结果</h2>';
