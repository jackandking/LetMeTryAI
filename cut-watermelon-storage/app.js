...
const questionConfig = {
    title: '切开的西瓜能放多久？家长都怎么处理',
    question: '你通常会怎么处理切开的西瓜？',
    options: [
        { value: 'eat-immediately', label: '马上吃完' },
        { value: 'refrigerate-short-term', label: '冰箱冷藏（隔夜内吃）' },
        { value: 'room-temp-few-hours', label: '常温短放（当天内）' },
        { value: 'freeze-for-later', label: '切块冷冻备用' }
    ],
    storageKey: 'cut_watermelon_storage.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=cut-watermelon-storage'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>切开的西瓜能放多久？家长都怎么处理结果</h2>';
