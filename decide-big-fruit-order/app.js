...
const questionConfig = {
    title: '遇到大份网购水果，家里怎么决定？家庭分配方式投票',
    question: '遇到大份网购水果，家里通常怎么决定分配？',
    options: [
        { value: 'even-share', label: '平均分给每人' },
        { value: 'preference-based', label: '按喜好优先分配' },
        { value: 'rotate-and-store', label: '轮流挑选或分期吃' },
        { value: 'share-with-neighbors', label: '分享给邻居或朋友' }
    ],
    storageKey: 'decide_big_fruit_order.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=decide-big-fruit-order'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>遇到大份网购水果，家里怎么决定？家庭分配方式投票结果</h2>';
