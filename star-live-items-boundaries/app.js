...
const questionConfig = {
    title: '明星直播的个人物品与互动边界：你怎么看',
    question: '在明星直播中，关于个人物品与互动边界，你更支持哪种态度或做法？',
    options: [
        { value: 'no-touch-no-requests', label: '尊重隐私不触碰或索要物品' },
        { value: 'host-initiated-sharing', label: '主播主动展示，粉丝不索要' },
        { value: 'safe-interaction-no-items', label: '互动可接受但不涉及私人物品' },
        { value: 'host-discretion', label: '根据主播意愿灵活处理' }
    ],
    storageKey: 'star_live_items_boundaries.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=star-live-items-boundaries'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>明星直播的个人物品与互动边界：你怎么看结果</h2>';
