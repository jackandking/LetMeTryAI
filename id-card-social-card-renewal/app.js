...
const questionConfig = {
    title: '身份证到期，社保卡要不要一起换？',
    question: '身份证到期时，是否需要同时更换社保卡？',
    options: [
        { value: 'change-both', label: '同时更换' },
        { value: 'change-id-only', label: '只换身份证' },
        { value: 'depends', label: '视情况而定' },
        { value: 'consult-family-community', label: '请家属或社区代办' }
    ],
    storageKey: 'id_card_social_card_renewal.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=id-card-social-card-renewal'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>身份证到期，社保卡要不要一起换？结果</h2>';
