...
const questionConfig = {
    title: '你和另一半是同床睡还是分床睡？你的睡眠画面是哪种？',
    question: '你和另一半是同床睡还是分床睡？',
    options: [
        { value: 'same-bed', label: '同床睡 — 喜欢靠着入睡' },
        { value: 'separate-beds', label: '分床睡 — 各自有独立空间' },
        { value: 'flexible-schedule', label: '灵活分配 — 看情况轮换' }
    ],
    storageKey: 'sleep_together_or_separate.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=sleep-together-or-separate'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>你和另一半是同床睡还是分床睡？你的睡眠画面是哪种？结果</h2>';
