...
const questionConfig = {
    title: '带孩子逛博物馆，哪个镇馆之宝会让他惊呼？',
    question: '带孩子逛博物馆，哪个镇馆之宝会让他惊呼？',
    options: [
        { value: 'moving-dinosaur-model', label: '会动的恐龙模型（仿真互动）' },
        { value: 'giant-mural-handscroll', label: '古代壁画或长卷（超大画面）' },
        { value: 'bronze-ritual-ding', label: '青铜大鼎（恢弘造型）' },
        { value: 'famille-rose-vase', label: '粉彩花瓶（精致彩绘）' }
    ],
    storageKey: 'kids_museum_treasure.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=kids-museum-treasure'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>带孩子逛博物馆，哪个镇馆之宝会让他惊呼？结果</h2>';
