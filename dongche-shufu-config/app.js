...
const questionConfig = {
    title: '动车旅行舒适配置选择',
    question: '出行时你会优先为孩子选择哪种动车旅行舒适配置？',
    options: [
        { value: 'window-seat-neck-pillow', label: '靠窗座位与颈枕' },
        { value: 'portable-snacks-thermos', label: '便携餐点与保温杯' },
        { value: 'tablet-offline-headphones', label: '离线平板与儿童耳机' },
        { value: 'organizer-wipes', label: '旅行收纳包与湿巾' }
    ],
    storageKey: 'dongche_shufu_config.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=dongche-shufu-config'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>动车旅行舒适配置选择结果</h2>';
