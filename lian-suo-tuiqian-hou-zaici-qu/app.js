...
const questionConfig = {
    title: '连锁店退钱后你还会再去吗？',
    question: '连锁店退钱后你还会再去吗？（遇到类似情况你会怎么选）',
    options: [
        { value: 'still-go-cautious', label: '会继续去（但更谨慎）' },
        { value: 'no-return', label: '不会再去了' },
        { value: 'order-only', label: '还会去，但只线上/点外卖' },
        { value: 'go-and-feedback', label: '会再去并当面反馈/投诉' }
    ],
    storageKey: 'lian_suo_tuiqian_hou_zaici_qu.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=lian-suo-tuiqian-hou-zaici-qu'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>连锁店退钱后你还会再去吗？结果</h2>';
