...
const questionConfig = {
    title: '你会为热门零食店排队吗？投票说说你的态度',
    question: '你会为热门零食店排队吗？',
    options: [
        { value: 'willing-to-queue', label: '愿意排队尝鲜' },
        { value: 'check-reviews-first', label: '先看评价再决定' },
        { value: 'wont-queue-for-snacks', label: '不会为零食排队' },
        { value: 'only-for-deals', label: '只为优惠或限定时排队' }
    ],
    storageKey: 'queue_for_popular_snack_shop.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=queue-for-popular-snack-shop'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>你会为热门零食店排队吗？投票说说你的态度结果</h2>';
