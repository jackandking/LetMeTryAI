...
const questionConfig = {
    title: '网红凤冠冰箱贴：你会怎么使用它？',
    question: '看到网红凤冠冰箱贴，你会怎么使用或展示它？',
    options: [
        { value: 'on-fridge-front', label: '贴在冰箱正面当装饰' },
        { value: 'on-fridge-side', label: '贴侧面或角落作低调点缀' },
        { value: 'clip-on-bag-clothes', label: '夹包包或衣物当时尚配件' },
        { value: 'photo-prop', label: '当拍照道具或平面摆拍' }
    ],
    storageKey: 'fengguan_fridge_sticker.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=fengguan-fridge-sticker'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>网红凤冠冰箱贴：你会怎么使用它？结果</h2>';
