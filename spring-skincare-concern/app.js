...
const questionConfig = {
    title: '春季护肤困扰调查',
    question: '换季期间，你的皮肤更容易出现哪种状况？',
    options: [
        { value: 'dry-flaky', label: '干燥脱皮' },
        { value: 'sensitive-redness', label: '敏感泛红' },
        { value: 'oily-acne', label: '出油长痘' },
        { value: 'dull-skin', label: '暗沉无光' }
    ],
    storageKey: 'spring_skincare_concern.data'
};
...
function showAd() {
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=spring-skincare-concern'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>春季护肤困扰调查结果</h2>';
