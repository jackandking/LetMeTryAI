...
const questionConfig = {
    title: '生孩子不花钱了，你会把省下的钱先用在哪儿？',
    question: '生孩子不花钱了，你会把省下的钱先用在哪儿？',
    options: [
        { value: 'education-invest', label: '孩子教育（补习 / 兴趣班）' },
        { value: 'family-travel', label: '家庭旅行 / 亲子活动' },
        { value: 'home-savings', label: '存起来 / 还房贷或买房' },
        { value: 'daily-needs', label: '日常花销 / 宝宝用品' }
    ],
    storageKey: 'sheng_hai_bu_hua_qian.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=sheng-hai-bu-hua-qian'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>生孩子不花钱了，你会把省下的钱先用在哪儿？结果</h2>';
