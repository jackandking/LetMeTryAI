...
const questionConfig = {
    title: '遇到突发事故你会先做哪一步？在家或路上遇到紧急情况',
    question: '遇到突发事故你会先做哪一步？（选出你的首要反应）',
    options: [
        { value: 'call-emergency', label: '拨打急救电话' },
        { value: 'call-for-help', label: '呼喊并召集周围人帮忙' },
        { value: 'contact-family', label: '联系家人或亲属' },
        { value: 'record-and-wait', label: '记录位置并等待救援' }
    ],
    storageKey: 'emergency_first_action.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=emergency-first-action'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>遇到突发事故你会先做哪一步？在家或路上遇到紧急情况结果</h2>';
