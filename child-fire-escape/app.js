...
const questionConfig = {
    title: '孩子遇到火灾，你会先做哪件事？',
    question: '孩子遇到火灾，你会先做哪件事？',
    options: [
        { value: 'carry-child-evacuate', label: '抱起孩子立即撤离' },
        { value: 'alert-and-evacuate-others', label: '先通知并疏散其他人' },
        { value: 'use-fire-extinguisher-or-isolate', label: '找灭火器或隔离火源' },
        { value: 'call-119-then-guide-child', label: '先拨打119并指引孩子逃生' }
    ],
    storageKey: 'child_fire_escape.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=child-fire-escape'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>孩子遇到火灾，你会先做哪件事？结果</h2>';
