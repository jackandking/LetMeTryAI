...
const questionConfig = {
    title: '酒店衣物收纳与保养投票 — 你的旅行衣物怎么处理？',
    question: '在酒店期间，你通常选择哪种方式收纳与保养随身衣物？',
    options: [
        { value: 'instant-folding', label: '即时折叠收纳' },
        { value: 'hang-on-hangers', label: '衣架挂放防皱' },
        { value: 'packing-cubes', label: '旅行收纳袋与分区' },
        { value: 'spot-cleaning-air', label: '局部清洁与通风' }
    ],
    storageKey: 'hotel_clothing_care.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=hotel-clothing-care'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>酒店衣物收纳与保养投票 — 你的旅行衣物怎么处理？结果</h2>';
