...
const questionConfig = {
    title: '你和伴侣是同床还是分床睡？揭秘老夫妻的睡眠选择',
    question: '你和伴侣是同床还是分床睡？请选择最贴近的一项',
    options: [
        { value: 'always-same-bed', label: '一直同床' },
        { value: 'same-bed-separated', label: '同床但分开睡姿' },
        { value: 'separate-beds', label: '长期分床' },
        { value: 'sometimes-separate', label: '偶尔分床' }
    ],
    storageKey: 'bedshare_or_separate_sleep.data'
};
...
function showAd() {
    ...
    if (typeof ks !== 'undefined' && ks.navigateTo) {
        ks.navigateTo({
            url: '/pages/showRewardedVideoAd/showRewardedVideoAd?result_page_id=bedshare-or-separate-sleep'
        });
        return;
    }
...
function showResult(latestVoteData) {
    ...
    resultDiv.innerHTML = '<h2>你和伴侣是同床还是分床睡？揭秘老夫妻的睡眠选择结果</h2>';
